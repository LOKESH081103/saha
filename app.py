...
<style>
    ...
    /* Column backgrounds - simple white boxes with borders */
...
with col_chola:
    st.markdown('<div class="section-header">Chola (Existing Loan)</div>', unsafe_allow_html=True)
    ...
    # Foreclosure & Rate Reset expander
    with st.expander("Foreclosure & Rate Reset", expanded=False):
    ...
    # Top-up Details & Principal Contribution - INSIDE Chola box
    st.markdown('<div class="section-header">Top-up Details & Principal Contribution</div>', unsafe_allow_html=True)
...
with col_nb:
    ...
    st.markdown('<div class="section-header">New Bank (Shift)</div>', unsafe_allow_html=True)
    
    # New Bank main inputs
    ...
    with r1:
        if auto_nb_prefill:
            ...
            st.markdown(f"**Loan Amt (POS + FC):** {fmt_inr(nb_loan_amt_input)}")
            st.caption("Auto-calculated NB Loan Amount (POS + FC). Top-up is separate.")
        else:
            nb_loan_amt_input = st.number_input("NB Loan Amt (POS + FC)", value=prefill_nb_loan_amt_base, format="%f")
        
        st.markdown('<div class="section-header" style="margin-top: 16px;">NB Details</div>', unsafe_allow_html=True)
        nb_mode_sel = st.radio("NB Calculation Mode", ["Keep Tenure same", "Keep EMI same", "Manual Tenure"], horizontal=True)
